...
df_purchase = pl.read_csv("User_product_purchase_details_p2.csv")
df_user = pl.read_csv("user_demographics.csv")

# Merge on User_ID
df = df_purchase.join(df_user, on="User_ID", how="left")
...
df = df.with_columns(
    (pl.col("Purchase") >= 10000).cast(pl.Int64).alias("High_Value_Purchase")
)

print("\nTarget distribution:")
print(df.select("High_Value_Purchase").group_by("High_Value_Purchase").agg(pl.len()))

# Drop columns not needed
df = df.drop(["Product_ID", "User_ID"])
...
df = df.to_dummies(columns=existing_categorical_cols, drop_first=True)
...
X = df.drop(["High_Value_Purchase", "Purchase"])
y = df.select("High_Value_Purchase")
